...
window.Bridge = (function () {
  ...
  async function readWorkspaceFile(id, name) { return await req("/v1/tasks/" + id + "/file?name=" + encodeURIComponent(name), "GET", null, settings.token); }
  // 公司长期记忆
  ...
  // 成就系统
  async function getAchievements() { return await req("/v1/achievements", "GET", null, settings.token); }
  // 公司等级/里程碑
  ...
  return {
    ...
    listTasks, listProjects, createTask, dispatchTask, setTaskStatus, setTaskPriority, cancelTask, deleteTask, archiveTask, restoreTask, sendFeedback, listArchived, listWorkspace, readWorkspaceFile, getMemory, getStats, getEconomy, getCompany, getDaily, getAchievements,
